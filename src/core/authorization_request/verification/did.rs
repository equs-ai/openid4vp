...
pub async fn verify_with_resolver(
    ...
    let headers_json_bytes = BASE64_URL_SAFE_NO_PAD.decode(headers_b64).map_err(|e| {
        Error::protocol_access_denied("jwt headers were not valid base64url", &state).add_source(e.into())
    })?;

    let mut headers =
        serde_json::from_slice::<Map<String, Json>>(&headers_json_bytes).map_err(|e| {
            Error::protocol_access_denied("jwt headers were not valid json", &state).add_source(e.into())
        })?;

    let Json::String(alg) = headers
        .remove("alg")
        .ok_or_else(|| Error::protocol_access_denied("'alg' was missing from jwt headers", &state))?
    else {
    ...
    let jwk = resolver.fetch_public_jwk(Some(&kid)).await.map_err(|e| {
        Error::protocol_access_denied("unable to resolve verification method from 'kid' header", &state)
            .add_source(e.into())
    })?;

    let _: Json = ssi::claims::jwt::decode_verify(&request_jwt, &jwk).map_err(|e| {
        Error::protocol_access_denied("request signature could not be verified", &state)
            .add_source(e.into())
    })?;
